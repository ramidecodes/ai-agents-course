...
- Treat every AI product as if it were “the model.”
- Use chat as a sequence of disconnected questions.
...
- Describe what the model does and what the surrounding product adds.
...
1. Distinguish a model, AI application, assistant, workflow, automation, and agent.
...
- Explain the model–assistant–agent distinction.
...
**Module purpose:** Replace vague or anthropomorphic ideas with a practical model that remains useful as products change.  
**Module outcome:** The learner can distinguish the underlying model from the product around it and choose an appropriate level of autonomy.

### Lesson 3 — Model, application, and assistant

**Runtime:** 4 minutes  
**Lesson objective:** Explain why a model, an AI product, and an assistant are related but not interchangeable.  
**Learner output:** A one-sentence explanation of the distinction.

#### Teaching points

- A model is the reasoning and generation engine, not the complete user experience.
- The application around the model adds the interface, account, files, search, history, integrations, permissions, and product-specific behavior.
- An assistant is the working experience created when a model is combined with instructions, context, product features, and available capabilities.
- Two products can use similar models but behave differently because they expose different context, tools, memory, or permissions.
- The same product may offer different models with different speed, cost, reasoning, or modality tradeoffs.
- “Multimodal” means the system can work with more than one form of information; it does not mean every product supports every modality in every workflow.

#### Suggested teaching flow

1. Show three nested layers: model → application → assistant experience.
2. Use the “brain inside a workplace” analogy.
3. Compare changing the model with changing the whole application.
4. Give a short example: the model may be capable of reading a document, but the product determines whether that document can be uploaded, searched, retained, or shared with tools.
5. Ask the retrieval question: “When I switch AI apps, what else changes besides the model?”
6. Close with the rule: **evaluate the complete system, not only the model name.**

#### Misconceptions to correct

- “ChatGPT,” “Claude,” or another product is simply one model.
- A newer or larger model automatically has access to more of the learner’s data.
...
Use a research assistant. The model can summarize text; the application determines whether it can search the web, open a private workspace, cite sources, or save a result.

#### Slide opportunities

- Three-layer diagram.
- Capability comparison: model capability vs. product capability vs. granted access.
- One-sentence retrieval check.

#### Transition

“An assistant can answer a request. An agent goes further by pursuing a goal across multiple steps—but that is a spectrum, not a magic category.”
